...
namespace Comet.Game.Database
{
    ...
    {    
        private static ServerConfiguration.DatabaseConfiguration _dbConfig;

        public static void Initialize(ServerConfiguration.DatabaseConfiguration dbConfig)
        {
            _dbConfig = dbConfig;
        }
        ...
        {
            if (!optionsBuilder.IsConfigured && _dbConfig != null)
            {
                var connectionString = $"server={_dbConfig.Hostname};database={_dbConfig.Schema};user={_dbConfig.Username};password={_dbConfig.Password}";
