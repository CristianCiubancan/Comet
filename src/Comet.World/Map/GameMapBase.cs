using Comet.Shared;
using Comet.World.Enums;
using Comet.World.Roles;
...
namespace Comet.World.Map
{
    ...
    {
        private static readonly ILogger logger = LogFactory.CreateLogger<GameMapBase<TRole, TUser>>();
        ...
        public bool UpdateMapData(uint mapDoc)
        {
            ...
            if (mapData == null)
            {
                logger.LogError($"Could not load map {Identity}({MapDoc}): map data not found");
                return false;
            }
